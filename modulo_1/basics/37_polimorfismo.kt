...
class Cheque(val numeroCheque: String, val banco: String) : Pagable {
    override val nombre = "Cheque"
    override fun procesar(monto: Double): Boolean {
        println("Procesando cheque $numeroCheque del banco $banco por $${"%.2f".format(monto)}")
        return true
    }
}
